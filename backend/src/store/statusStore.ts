...
const store: Status[] = [
  ...
  },
  {
    ...
    message: 'We are seeing a 3% error rate on the /v2/payments endpoint. Investigation underway.',
    severity: 'high',
...
export function createStatus(title: string, message: string, severity: Severity): Status {
  const entry: Status = {
    id: randomUUID(),
    title: title.trim(),
